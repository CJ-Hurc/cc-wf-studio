...
SURFACE = "capability:vscode-extension"
SCENARIOS = frozenset({"activation", "render", "failure-state"})
...
def _prove_cell(
	*,
	...
	if not out.is_file():
		print(
			f"vscode-view-pack: no receipt for {cid} rc={proc.returncode} "
	...
	if not ok:
		print(
			f"vscode-view-pack: cell {cid} scenario={cell['scenario']} "
			f"failed: {data}",
			file=sys.stderr,
		)
	return ok
...
def main(argv: list[str] | None = None) -> int:
	ap = argparse.ArgumentParser(description="cc-wf-studio vscode_view pack prove")
	ap.add_argument("--project-dir", default="")
	ap.add_argument("--sandbox-url", default="")
	ap.add_argument("--pack", default="vscode_view")
	...
	if not _ensure_sandbox(root, sandbox):
		print(f"vscode-view-pack: sandbox unhealthy at {sandbox}", file=sys.stderr)
		return 1
	...
	if active is None:
		payload = {
			"schema": "hurc-complete-e2e-vscode-view-pack/v1",
			"ok": True,
			"sandbox_url": sandbox,
			"notes": "sandbox healthy; no active run to drain",
			"proven": 0,
			"checks": ["viewActivation", "viewRender", "viewFault"],
			"prover": "product-view-pack-cdp",
		}
	else:
		...
		if cells and (adapter is None or runner is None):
			print(
				"vscode-view-pack: missing harness adapter or bun/tsx runner",
				file=sys.stderr,
			)
			return 1
		for cell in cells:
		...
		# Attach campaign identity from first cell when present (harness identity check).
		if cells:
			payload["snapshot_id"] = cells[0].get("snapshot_id")
			payload["universe_id"] = cells[0].get("universe_id")

	out_path = Path(args.out) if args.out else (
		root / ".hurc-harness" / "state" / "complete-e2e" / "vscode-view-pack-vscode_view.json"
	)
